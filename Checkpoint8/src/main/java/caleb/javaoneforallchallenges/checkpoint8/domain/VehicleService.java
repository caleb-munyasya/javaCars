...
public class VehicleService {
        ...
        if (vehicleToUpdate != null) {
            ...
            if (featureToUpdate == 5) {
                ...
                while (proceed != 0 && proceed != 1) {
                    System.out.println("In order to change the vehicle type, this one must be deleted and you can then create a new vehicle.");
                ...
                if (proceed == 1) {
                    deleteVehicleByModel(replacementVehicle.getModel());
                }
    ...
    public void addNewVehicle() {
        ...
        while (vehicleType < 1 || vehicleType > 6) {
            ...
            if (vehicleType == 1) {
                vehicleRepository.saveToDatabase(new Car(newModel, newColor, newYear, newAutomaker));
            }
            if (vehicleType == 2) {
                vehicleRepository.saveToDatabase(new Motorcycle(newModel, newColor, newYear, newAutomaker));
            }
            if (vehicleType == 3) {
                vehicleRepository.saveToDatabase(new Van(newModel, newColor, newYear, newAutomaker));
            }
            if (vehicleType == 4) {
                vehicleRepository.saveToDatabase(new Truck(newModel, newColor, newYear, newAutomaker));
            }
            if (vehicleType == 5) {
                vehicleRepository.saveToDatabase(new Pickup(newModel, newColor, newYear, newAutomaker));
            }
            if (vehicleType == 6) {
                vehicleRepository.saveToDatabase(new Others(newModel, newColor, newYear, newAutomaker));
            }
        }
    }
}
